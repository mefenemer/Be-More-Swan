...
window._calOpenPost = async function (postId) {
    ...
    if (post.linkUrl || post.ctaText) {
        ...
        if (post.linkUrl) {
            // _escHtml stops markup, not a `javascript:` href — and this value is user-supplied.
            // Anything that isn't a plain http(s) address is shown as text instead of a link.
            const href = /^https?:\/\/[^\s]+$/i.test(String(post.linkUrl).trim()) ? String(post.linkUrl).trim() : null;
            linksContent.innerHTML += href
                ? `<a href="${_escHtml(href)}" target="_blank" rel="noopener" class="flex items-center gap-1 text-xs text-blue-600 hover:underline truncate">
                ...
                : `<p class="text-xs text-gray-400 truncate">${_escHtml(post.linkUrl)}</p>`;
        }
        if (post.ctaText) {
