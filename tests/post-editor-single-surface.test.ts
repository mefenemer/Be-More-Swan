...
check('every block the rail borrows still has a home to be restored to', () => {
    ...
    const rail = workspace.slice(landmark(workspace, 'const _RAIL = ['), landmark(workspace, '\n];', landmark(workspace, 'const _RAIL = [')));
...
check('the rail steps run content first, with no Platforms step', () => {
    const block = workspace.slice(landmark(workspace, 'const _RAIL = ['), landmark(workspace, '\n];', landmark(workspace, 'const _RAIL = [')));
...
check('a new post is seeded across every connected platform', () => {
    ...
    const seed = workspace.slice(landmark(workspace, 'async function pceOpenDestinationsForCreate('));
    ...
    const confirm = workspace.slice(landmark(workspace, 'async function pceConfirmDestinations()'));
...
check('step 3 is video-only and says so, and stills keep their text and sound', () => {
    ...
    const imgStep = block.slice(landmark(block, "key: 'imgtext'"), landmark(block, "key: 'video'"));
    assert.match(imgStep, /pce-insp-audio/, 'a photo with a voice note publishes as a video — sound goes with it');
    const mediaStep = block.slice(landmark(block, "key: 'media'"), landmark(block, "key: 'imgtext'"));
    ...
    const toggle = workspace.slice(landmark(workspace, 'function _railToggle(key)'));
...
check('the format blocker survived the deleted step, somewhere always visible', () => {
    const body = workspace.slice(landmark(workspace, 'id="post-review-media-alert"'), landmark(workspace, 'id="post-review-tabs"'));
...
check('the header states no format until there is media to route', () => {
    const fn = workspace.slice(landmark(workspace, 'function _rqReviewRenderTabs()'));
    const tabsBody = fn.slice(0, landmark(fn, '\n}\n'));
...
check('the modal owns the scroll while it is open', () => {
    const open = workspace.slice(landmark(workspace, "const modal = document.getElementById('post-review-modal');"));
    ...
    const close = workspace.slice(landmark(workspace, 'function closePostReview()'));
...
check('step 7 is the decision, with three ways to commit', () => {
    const at = landmark(workspace, 'id="post-review-reschedule"');
    const panel = workspace.slice(at, landmark(workspace, 'id="post-review-disclosure"', at));
    ...
    const fn = workspace.slice(landmark(workspace, 'async function rqReviewScheduleMyself()'));
...
check('there is exactly one rqReviewPublishNow, and it is the one with the gates', () => {
    ...
    const fn = workspace.slice(landmark(workspace, 'async function rqReviewPublishNow()'));
...
check('the commit actions show they are working, and cannot be double-fired', () => {
    ...
    const wrap = workspace.slice(landmark(workspace, 'async function _rqApproveTargets('));
    ...
    for (const fn of ['rqReviewApprove()', 'rqReviewScheduleMyself()', 'rqReviewPublishNow()']) {
        ...
        const tag = workspace.slice(at, landmark(workspace, '>', at));
...
check('an unsaved card edit is landed, not discarded, when you click away', () => {
    ...
    assert.ok(landmark(body, '_pceFlushCardSave()') < landmark(body, '_rqReviewPostId = null'),
    ...
    const sync = workspace.slice(landmark(workspace, 'const sameCardStillPending'));
    ...
    const save = workspace.slice(landmark(workspace, 'async function _pceSaveCardNow()'));
...
check('both drafting buttons are in step 1, not the modal header', () => {
    const block = workspace.slice(landmark(workspace, 'id="pce-write-block"'), landmark(workspace, 'id="post-review-amend-all"'));
    ...
    const header = workspace.slice(landmark(workspace, 'id="post-review-title"'), landmark(workspace, '<!-- Body: platform mock-up'));
...
check('every text box the user writes into has a mic the layout actually shows', () => {
    const fields = workspace.slice(landmark(workspace, 'const GP_VOICE_FIELDS = {'), landmark(workspace, 'function gpStartVoice'));
    ...
    const opener = workspace.slice(landmark(workspace, 'function _pceOpenInlineCaption'));
...
check('the Media step carries the style chooser and the whole media inspector', () => {
    const railBlocks = workspace.slice(landmark(workspace, 'const _RAIL = ['), landmark(workspace, '\n];', landmark(workspace, 'const _RAIL = [')));
...
check('the caption length badges no longer carry media warnings', () => {
    const fn = workspace.slice(landmark(workspace, 'function _pceRefreshCaptionMeta'));
...
check('the caption badges say nothing on an empty post, and step 6 carries the verdict', () => {
    ...
    const sub = workspace.slice(landmark(workspace, "case 'check': {"));
    ...
    const shared = workspace.slice(landmark(workspace, 'function _pceOverLimitPlatforms('), landmark(workspace, 'function _pceRefreshCaptionMeta'));
...
check('the mic toggles, tracks one session, and reports real failures', () => {
    const fn = workspace.slice(landmark(workspace, 'let _gpVoice = null;'), landmark(workspace, '// ── Shared chip styling'));
    ...
    const commit = workspace.slice(landmark(workspace, 'async function _pceCommitInlineCaption'));
...
check('a post you are writing offers save-and-discard, not reject-and-rewrite', () => {
    ...
    const sync = workspace.slice(
        landmark(workspace, "const isOwnDraft = post.status === 'draft';"),
        landmark(workspace, '// Per-post AI disclosure footer'));
    ...
    const discard = workspace.slice(landmark(workspace, 'async function rqReviewDiscardDraft()'));
    ...
    const save = workspace.slice(landmark(workspace, 'async function rqReviewSaveDraft()'));
...
check('Check & improve and Schedule & publish are gated on one readiness rule', () => {
    ...
    const fn = workspace.slice(landmark(workspace, 'function _pceNotReadyReason(post)'));
    ...
    const subWhen = workspace.slice(landmark(workspace, "case 'when': {"));
...
check('a draft shows no scheduled time anywhere', () => {
    ...
    const sub = workspace.slice(landmark(workspace, "case 'when': {"));
...
check('the empty caption carries its own Dictate button', () => {
    ...
    const click = workspace.slice(landmark(workspace, 'function _pceCanvasClick(ev)'));
...
check('step 1\'s rewrite actions are disabled until there is a caption', () => {
    const block = workspace.slice(landmark(workspace, 'id="pce-write-block"'), landmark(workspace, 'id="pce-write-freetext"'));
    ...
    const render = workspace.slice(landmark(workspace, 'function _railRender()'));
    assert.match(render.slice(0, 6000), /_pceSyncRewriteActions\(\)/, 'the step can be opened at any time');
    const meta = workspace.slice(landmark(workspace, 'function _pceRefreshCaptionMeta()'));
...
check('nothing brings the pre-rail layer list back', () => {
    const fn = workspace.slice(landmark(workspace, 'function _pceRenderLayers()'));
    const body = fn.slice(0, 1800);
...
check('every media source in the picker has a glossary entry', () => {
    ...
    const at = landmark(assistants, 'const _MEDIA_SOURCE_META = {');
    const meta = assistants.slice(at, landmark(assistants, '\n};', at));
...
check('the media sourcing panel lives in its own modal', () => {
    ...
    const close = workspace.slice(landmark(workspace, 'function _pceCloseMediaPicker()'));
...
check('a branded card is offered where you look for a picture, and only once', () => {
    ...
    const open = workspace.slice(landmark(workspace, 'function _pceOpenMediaPicker()'));
...
check('the card paths refetch by the post\'s own status', () => {
    ...
    const fn = workspace.slice(landmark(workspace, 'async function _pceCreateBrandCard()'));
    const body = fn.slice(0, 2500);
...
check('the headline is placeable and editable on the canvas', () => {
    ...
    const paint = workspace.slice(landmark(workspace, 'function _pcePaintHandles()'));
    ...
    const drag = workspace.slice(landmark(workspace, 'function _pceDragStart(ev)'));
...
check('every card change commits without a Save button', () => {
    ...
    const bind = workspace.slice(landmark(workspace, '(function _pceBind()'));
    ...
    const dragEnd = workspace.slice(landmark(workspace, 'function _pceDragEnd('));
    ...
    const reset = bind.slice(landmark(bind, "_pceEl('pce-resetkit')"));
...
check('a drag handle is an outline, not a caption printed on the card', () => {
    const paint = workspace.slice(landmark(workspace, 'function _pcePaintHandles() {'));
    const body = paint.slice(0, landmark(paint, '\n}\n'));
...
check('a still image is not offered Sound', () => {
    const sync = workspace.slice(landmark(workspace, 'const audioCapable ='));
    const line = sync.slice(0, landmark(sync, '\n'));
    ...
    const rail = workspace.slice(landmark(workspace, "{ key: 'imgtext'"));
...
check('the mic button cannot fail silently', () => {
    const fn = workspace.slice(landmark(workspace, 'function gpStartVoice('));
    const head = fn.slice(0, landmark(fn, '_gpBeginVoice(targetId, cfg);'));
...
check('dragging a card element no longer snaps to thirds', () => {
    ...
    const move = workspace.slice(landmark(workspace, 'function _pceDragMove(ev) {'));
    const body = move.slice(0, landmark(move, '\n}\n'));
...
check('alignment clears a free x rather than silently doing nothing', () => {
    ...
    const bind = workspace.slice(landmark(workspace, '(function _pceBind()'));
    const align = bind.slice(landmark(bind, ".pce-align'"));
...
check('the client normalises x exactly as the renderer does', () => {
    const norm = workspace.slice(landmark(workspace, 'function _pceNormalizeLayout(raw) {'));
    const body = norm.slice(0, landmark(norm, '\n}\n'));
...
check('openPostReview warms the cache for every status the editor accepts', () => {
    const fn = workspace.slice(landmark(workspace, 'async function openPostReview(postId, opts = {})'));
    const warm = fn.slice(0, landmark(fn, 'const post = _rqPostCache[postId];'));
...
check('the footer stops inviting a published post to schedule itself', () => {
    const sync = workspace.slice(landmark(workspace, 'function _pceSyncForPost(post) {'));
...
check('saving a new caption invalidates the post cache it repaints from', () => {
    ...
    for (const fn of ['rqAcceptVerifyRewrite', 'rqApplySuggestions']) {
        const body = workspace.slice(landmark(workspace, `window.${fn} = async function`));
